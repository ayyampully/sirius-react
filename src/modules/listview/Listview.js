...
class Listview extends Component {
  ...
  componentWillMount() {
    let url = `http://localhost:3030/api/v1/getticketslist?projectid=${
  ...
  render() {
    if (!this.state.ticketList)
      return <div className="list-wrapper">Loading</div>;
    let rows = [];
        ...
        <tr key={ticket.ticketid}>
          ...
          <td>
            <Link className="view-more" to={url}>
              <span>{ticket.ticketid}</span>- {ticket.title}
            </Link>
          </td>
    ...
    return (
      <div className="list-wrapper">
        <aside className="nav-list">
          ...
          <h6 className="inner-title-sub">
            Admin: {this.state.ticketList.owner.name}
          </h6>
          <button className="btn">Add new item</button>
          <ul className="modules">
            <li>
              User Module <span>3</span>
            </li>
          </ul>
        </aside>

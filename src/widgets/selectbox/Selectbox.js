...
class Selectbox extends Component {
  ...
  render() {
    ...
    this.autoClt.forEach((item, i) => {
      items.push(
        <li key={item.label+i} onClick={this.setLabel.bind(this, item.label)}>{item.label} <i className={item.className} aria-hidden="true"></i></li>
      )
    });
    return (
      <div onClick={this.showAutoComplete.bind(this)} className="autocomplete-wrap" >
      <div ><span className="capitalize">{this.state.selectedItem}
      </span><i className="fa fa-chevron-down drop-arrow" aria-hidden="true"></i></div>

...
class Comments extends Component {
  ...
  render() {
    ...
    this.props.comments.forEach((comment, i) => {
      ...
      comments.push(
        <div key={comment.createddate + i} className="comment"> <span className="comment-icon">RA</span> 
          <div className="comment-body">
            <label className="user"><span>{comment.createdby.name}</span> wrote:</label>
    ...
    return (
      ...
      <div className="comments-add-form">
        <textarea placeholder="Type here..." className="text" id="comments"></textarea>
        <button onClick={this.addCommentHandler} className="btn">Send</button>
      </div>
    </div>
    );
  }
...
function formatCommentTime(date){
  ...
  if(diff<30){
    ...
    const dateString = tempDate.getMonth()+'/'+ tempDate.getDate()+'/'+tempDate.getFullYear();
    return dateString;
  }
}
